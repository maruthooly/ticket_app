...
import { assets ,dummyDateTimeData, dummyShowsData } from '../assets/asset';
...
const SeatLayout =() =>{
  ...
  const groupRows = [["A","B"],["C","D"],["F","E"],["G","H"],["I","J"]]
    ...
    const handleSeatClick = (seatId)=>{
      ...
      //select chair to see movie
      if(!selectedSeats.includes(seatId) && selectedSeats.length > 5){
        return toast("ํYour can only select 5 seats")
      }
            ...
            {Array.from({ length: count}, (_, i) => {
              ...
              return(
                <button key={seatId} onClick={() => handleSeatClick
                  (seatId)} className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId) && 
    ...
    return show ?(
      <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
       ...
       <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
          ...
          <button onClick={() => navigate('/my-bookings')} className='flex items-center gap-1 mt-20 px-10 py-3
